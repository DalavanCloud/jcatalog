...
def match_scielo():
    for doc in models.Scopus.objects():
        ...
        if doc.is_scielo == 0:
            try:
                docsci = models.Scielo.objects.get(
                    title_at_scielo_country__iexact=doc.title_country)
                doc.modify(
                    is_scielo=1,
                    scielo_id=str(docsci.id),
                    updated_at=datetime.datetime.now)
...
def match_scimago():
    ...
    for doc in docscopus:
        if doc.is_scimago == 0:
            for issn in doc.issn_list:
                try:
                    ...
                    docsmago = models.Scimago.objects.filter(
                        issn_list=issn).batch_size(5)
                    doc.modify(
                        is_scimago=1,
        ...
        if doc.is_scimago == 0:
            try:
                docsmago = models.Scimago.objects.get(
                    title_country__iexact=doc.title_country)
                doc.modify(
                    is_scimago=1,
                    scimago_id=str(docsmago.id),
                ...
                docsmago = models.Scimago.objects.filter(
                    title_country__iexact=doc.title_country).batch_size(5)
                doc.modify(
                    is_scimago=1,
                    scimago_id=str(docsmago[0].id),
                    updated_at=datetime.datetime.now)
